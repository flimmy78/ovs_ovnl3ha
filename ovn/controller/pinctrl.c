            ...
            struct sset *active_tunnels)
{
    ...
    send_garp_run(br_int, chassis, lports, local_datapaths, active_tunnels);
}
                     ...
                     struct sset *active_tunnels)
{
    ...
    if (!extract_addresses_with_port(nat_address, laddrs, &lport)
        ...
        || (lport && !pinctrl_is_chassis_resident(
            lports, chassis, lport, active_tunnels))) {
        destroy_lport_addresses(laddrs);
        free(laddrs);
        free(lport);
        return;
              ...
              struct sset *active_tunnels)
{
    ...
    get_nat_addresses_and_keys(&nat_ip_keys, &local_l3gw_ports, lports,
                               chassis, &nat_addresses, active_tunnels);
