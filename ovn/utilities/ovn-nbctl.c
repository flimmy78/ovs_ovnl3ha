...
nbctl_lrp_get_gateway_chassis(struct ctl_context *ctx)
{
    ...
    for (i = 0; i < lrp->n_gateway_chassis; i++) {
        const struct nbrec_gateway_chassis *gc = lrp->gateway_chassis[i];
        smap_add_format(&gcs, gc->name, UUID_FMT " (%s)",
                        UUID_ARGS(&gc->header_.uuid), gc->name);
    }
    const struct smap_node **nodes = smap_sort(&gcs);
    for (i = 0; i < smap_count(&gcs); i++) {
        const struct smap_node *node = nodes[i];
        ds_put_format(&ctx->output, "%s\n", node->value);
    }
    smap_destroy(&gcs);
...
static const struct ctl_command_syntax nbctl_commands[] = {
    ...
    { "lrp-del-gateway-chassis", 2, 2, "PORT CHASSIS", NULL, nbctl_lrp_del_gateway_chassis,
      NULL, "", RW },
